...
class WhisperModel:
    ...
    def transcribe_stream(
        ...
        """
        真正的流式转录接口。
        接收任意长度的音频 chunk 流 (16kHz, mono, float32)，
        内部自动进行流式 VAD 切句、推理、上下文传递与绝对时间戳对齐。

        Args:
            audio_stream: 音频生成器/迭代器，yield np.ndarray (float32, 16kHz, mono)。
                          每次 yield 的长度不限，内部会自动缓冲并按 512 samples 喂给 VAD。
            vad_filter: 是否启用流式 VAD。强烈建议开启。
            其他参数: 与原版 transcribe() 完全一致。

        Yields:
            Segment: 带有绝对全局时间戳的识别结果。
        """
        sampling_rate = self.feature_extractor.sampling_rate
        ...
        if language is None:
            if not self.model.is_multilingual:
                ...
                # 流式场景下无法像离线那样取前30秒检测语言
                # 默认回退到中文或英文，实际使用建议显式传入 language
                language = "zh" 
                self.logger.warning(
                    "Language not specified in stream mode. Defaulting to '%s'. "
                    "Please specify language explicitly for best results.", language
                )
        else:
            if not self.model.is_multilingual and language != "en":
                self.logger.warning(
                    "The current model is English-only but language='%s'; using 'en'.", language
                )
                language = "en"
        ...
        # 3. 组装 TranscriptionOptions (完整复刻原版参数)
        options = TranscriptionOptions(
            beam_size=beam_size,
            best_of=best_of,
            patience=patience,
            ...
            suppress_tokens=(
                get_suppressed_tokens(tokenizer, suppress_tokens)
                if suppress_tokens else suppress_tokens
            ),
            without_timestamps=without_timestamps,
            max_initial_timestamp=max_initial_timestamp,
            word_timestamps=word_timestamps,
            prepend_punctuations=prepend_punctuations,
            append_punctuations=append_punctuations,
            multilingual=False,  # 流式不支持逐段语言检测
            max_new_tokens=max_new_tokens,
            clip_timestamps="0", # 流式不需要 clip
            hallucination_silence_threshold=hallucination_silence_threshold,
            hotwords=hotwords,
        )
        ...
        vad_iterator = StreamingVADIterator(vad_parameters, sampling_rate=sampling_rate)

        # 5. 状态维护
        previous_text = ""
        if isinstance(initial_prompt, str):
            previous_text = initial_prompt
            
        chunk_buffer = np.array([], dtype=np.float32)
        VAD_WINDOW_SIZE = 512  # Silero VAD 固定要求 512 samples (32ms @ 16kHz)

        # 6. 主流式循环
        for raw_chunk in audio_stream:
            if not isinstance(raw_chunk, np.ndarray) or raw_chunk.ndim != 1:
                raise ValueError("audio_stream must yield 1D numpy arrays (float32, 16kHz, mono)")
                
            chunk_buffer = np.append(chunk_buffer, raw_chunk)

            # 将外部任意长度的 chunk 切割为 VAD 所需的 512 samples
            while len(chunk_buffer) >= VAD_WINDOW_SIZE:
                vad_chunk = chunk_buffer[:VAD_WINDOW_SIZE]
                chunk_buffer = chunk_buffer[VAD_WINDOW_SIZE:]

                # 调用流式 VAD 状态机
                vad_result = vad_iterator(vad_chunk)

                if vad_result is not None:
                    speech_audio, abs_start, abs_end = vad_result

                    # 提取 Mel 频谱图特征
                    features = self.feature_extractor(speech_audio)

                    # 动态注入上下文 Prompt
                    if condition_on_previous_text and previous_text:
                        options.initial_prompt = previous_text

                    # 调用核心推理引擎
                    for segment in self.generate_segments(
                        features, tokenizer, options, log_progress=False
                    ):
                        # 【关键】时间戳映射：模型输出是相对时间，加上 VAD 提供的绝对起始时间
                        segment.start = round(segment.start + abs_start, 3)
                        segment.end = round(segment.end + abs_start, 3)

                        if segment.words:
                            for w in segment.words:
                                w.start = round(w.start + abs_start, 3)
                                w.end = round(w.end + abs_start, 3)

                        yield segment
                        previous_text += segment.text

                    # 防止 Prompt 超出 Whisper max_length 限制导致性能下降
                    MAX_PROMPT_CHARS = 800 
                    if len(previous_text) > MAX_PROMPT_CHARS:
                        punctuations = ['。', '！', '？', '.', '!', '?', '\n', '，', ',']
                        truncated = previous_text[-MAX_PROMPT_CHARS:]
                        
                        cut_idx = -1
                        for p in punctuations:
                            idx = truncated.find(p)
                            if idx != -1:
                                cut_idx = idx + 1 # 保留标点之后的内容
                                break
                                
                        if cut_idx > 0:
                            previous_text = truncated[cut_idx:]
                        else:
                            space_idx = truncated.rfind(' ')
                            if space_idx > 0:
                                previous_text = truncated[space_idx+1:]
                            else:
                                previous_text = truncated

        # 7. 处理流结束时 buffer 中剩余的音频
        if len(chunk_buffer) > 0:
            # 用零填充到 512 的倍数，喂给 VAD 做最后的 flush
            pad_size = VAD_WINDOW_SIZE - (len(chunk_buffer) % VAD_WINDOW_SIZE)
            if pad_size < VAD_WINDOW_SIZE:
                chunk_buffer = np.pad(chunk_buffer, (0, pad_size))
            ...
            while len(chunk_buffer) >= VAD_WINDOW_SIZE:
                ...
                if vad_result is not None:
                    speech_audio, abs_start, _ = vad_result
                    features = self.feature_extractor(speech_audio)
                    if condition_on_previous_text and previous_text:
                        options.initial_prompt = previous_text
                        
                    for segment in self.generate_segments(features, tokenizer, options, log_progress=False):
                        segment.start = round(segment.start + abs_start, 3)
                        segment.end = round(segment.end + abs_start, 3)
                        if segment.words:
                            for w in segment.words:
